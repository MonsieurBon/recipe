...
- State-changing admin actions on another account — deactivating a user today, and (planned) changing a role or deleting an account — are constrained by two server-side invariants. An admin may not apply them to their **own** account (a footgun with no legitimate use; the corresponding control is also disabled in the UI, keyed to the caller's immutable id). And no action may leave **zero active admins**: the last-active-admin check takes a write lock on the active-admin rows for the duration of the transaction, so two admins acting on each other concurrently cannot both pass and lock everyone out of administration. A refused action returns **409** with a machine-readable reason the client turns into a specific message rather than a generic error. A deactivation takes effect immediately: the account is refused at login and at refresh, and its outstanding access tokens are revoked via a token-version bump, so a disabled session can neither continue nor renew itself
- The admin user listing resolves its search, filter, and sort **server-side**. The sort key is restricted to an allow-list of non-sensitive columns (username, email, active state); any other key falls back to a stable identity ordering, so the order of results can never be used as an oracle to infer a sensitive column such as the password hash. The free-text term is matched with parameter-bound patterns whose `LIKE` wildcards are escaped (a literal `%` or `_` matches itself), and the projection returned to the client omits the password entirely
...
- Each user has a server-side **token version** embedded in their tokens; it is bumped whenever the user's access must be revoked (e.g. a role change or an account deactivation). A request authenticates only while the access token's version still matches the user's current version, so revocation invalidates outstanding access tokens within a short, bounded window. The current version is read through a short-lived local cache to avoid a database lookup on every request
- Refresh is not version-gated: it re-reads the user, so the access token it mints always carries the user's current role. A role change therefore takes effect immediately without forcing a re-login: revoking the outstanding access tokens makes the user's next request fail authentication, and the client's automatic refresh silently replaces the token with one carrying the new role. A demotion leaves no window in which the old privileges still apply. A refresh for a **disabled** account is refused outright, so a deactivation ends the session rather than letting the rolling refresh renew it indefinitely
