...
- Refresh is not version-gated: it re-reads the user, so a mid-session role change is reflected in the next access token — a demotion downgrades the session on the next refresh rather than forcing an immediate re-login. A refresh for a **disabled** account is refused outright, so a deactivation ends the session rather than letting the rolling refresh renew it indefinitely
